    ...
    <section id="products" className="bg-white px-5 py-20">
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-4 max-w-3xl leading-7 text-black/60">
          Browse {catalogProducts.length} Hikvision products. Prices include the current market adjustment and will be confirmed when we call you.
        </p>
          ...
          {categories.map((category) => {
            ...
            return <div key={category}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {products.map((product) => <article key={`${product.name}-${product.price}`} className="flex min-h-52 flex-col rounded-2xl border border-black/10 bg-cream p-5 shadow-sm">
